...
function yaml_dump_iter(html, data, indent, remove_meta) {
    for (const attr in data) {
        if (remove_meta && (attr[0] == '_' ||
            attr.substr(attr.length - 2) == '__' || attr == 'yaml')) continue;
        let value = data[attr];
        const type = typeof value;
        if (type === 'string') value = wrap_string(value, indent);
...
function obj_diff_iter(data0, data1) {
    for (const attr of Object.keys(data0)) {
        ...
        if (type0 === 'object' && Array.isArray(value0) && Array.isArray(value1)) {
            ...
            data0[attr] = value0;
            data1[attr] = value1;
            if (data0[attr].length === 0) delete data0[attr];
            if (data1[attr].length === 0) delete data1[attr];
        }

        if (type0 === 'object' && !Array.isArray(value0) && !Array.isArray(value1)) {
            obj_diff_iter(value0, value1);
            if (empty(data0[attr])) delete data0[attr];
            if (empty(data1[attr])) delete data1[attr];
        }
    }
}
