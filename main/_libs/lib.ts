...
function el(selector, wc?) {
    if (selector === undefined) throw 'Undefined'
    const obj = (wc === undefined) ? document : wc.shadow
    const els = obj.querySelectorAll(selector)
    if (els.length === 0) throw 'Element not exists: ' + selector
    if (els.length > 1) throw 'More than one elements: ' + selector
...
function valid(value) {
    ...
    if (typeof value === typeof {})
        for (const attr in value)
            if (value[attr] === undefined) throw 'Undefined'
...
function set_url_params(params) {
    ...
    for (const param in params) {
        if (params[param] === '' || params[param] === null || params[param] === undefined) continue
...
function datetime_format(dt: string | null) {
    if (dt == null) return ''
...
function datetime_diff_format(dt_iso: string | null) {
...
function array_index(arr, tag) {
    ...
    for (const item of arr) {
        if (!(item[tag] in ind)) ind[item[tag]] = []
