...
export class Module {
    ...
    init_child_IDs() {
        if (!this.ID) throw 'ID undefined'
        for (const attr in this) {
            const m: any = this[attr]
    ...
    el(element_id) {
        ...
        if (elem == undefined) throw 'Unknown element ID: ' + this.ID + '-' + element_id
        return elem;
    }
    ...
    check_IDs_iter(ids) {
        for (const attr in this) {
            const m: any = this[attr]
            if (m instanceof Module) {
                if (m.ID == undefined) throw 'Undefined id'
                if (ids.has(m.ID)) throw 'Duplicated id:' + m.ID
                ids.add(m.ID)
